      ...
      >
        ...
        {docType === 'insurance' && (
          <div style={{ background: '#fef3c7', color: '#92400e', textAlign: 'center', padding: '6px 12px', fontWeight: 700, fontSize: 12, letterSpacing: '0.04em' }}>
            🏷 {t('components.printDoc.insuranceCopyBadge')}
          </div>
        )}
        ...
        <div style={{ padding: '24px 32px 18px', borderBottom: '3px solid #0369a1' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <img src={rasoulLogo} alt={t('components.printDoc.hospitalName')} style={{ width: 64, height: 64, flexShrink: 0, objectFit: 'contain' }} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: '#0f172a' }}>{t('components.printDoc.hospitalName')}</div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 2 }}>{t('components.printDoc.hospitalDept')}</div>
            </div>
        ...
        <div style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0', padding: '12px 32px', display: 'flex', gap: 32 }}>
          {[[t('components.printDoc.fieldPatient'), patient?.name || '—'], [t('components.printDoc.fieldMrn'), patient?.mrn || '—'], [t('components.printDoc.fieldDate'), today], [t('components.printDoc.fieldAge'), patient?.age ? t('components.printDoc.ageYears', { age: patient.age }) : '—']].map(([l, v]) => (
            <div key={l}>
        ...
        <div style={{ margin: '0 32px 28px', borderTop: '1px solid #e2e8f0', paddingTop: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div style={{ fontSize: 11, color: '#94a3b8' }}>
            <div>{t('components.printDoc.generatedLabel')}: {today}</div>
            <div>{t('components.printDoc.hospitalName')} — {t('components.printDoc.hospitalDept')} · {t('components.printDoc.officialClinicalDocument')}</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 10, color: '#64748b' }}>{t('components.printDoc.attendingPhysician')}</div>
          </div>
        </div>
        ...
        <div className="no-print" style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0', padding: '14px 32px', display: 'flex', gap: 10, justifyContent: 'flex-end', borderRadius: '0 0 16px 16px' }}>
          <button onClick={onClose} style={{ padding: '9px 20px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: 13, color: '#475569' }}>{t('components.printDoc.close')}</button>
          <button onClick={handlePrint} style={{ padding: '9px 20px', borderRadius: 8, border: 'none', background: 'linear-gradient(135deg,#0369a1,#6366f1)', cursor: 'pointer', fontWeight: 700, fontSize: 13, color: '#fff', display: 'flex', alignItems: 'center', gap: 6 }}>
            🖨 {docType === 'insurance' ? t('components.printDoc.printInsuranceDocument') : t('components.printDoc.printDocument')}
          </button>
        </div>
      </div>
    </div>
